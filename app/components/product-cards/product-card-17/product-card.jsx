import Link from "next/link";
import Image from "next/image";
import Typography from "@mui/material/Typography";

// LOCAL CUSTOM COMPONENTS
import Discount from "./discount";
//import HoverActions from "./hover-actions";

// STYLED COMPONENTS
import { ImageWrapper, ContentWrapper, StyledCard } from "./styles";

// CUSTOM DATA MODEL

// CUSTOM UTILS FUNCTION
import PurchaseBtn from "./purchase-btn";
import { currency } from "../../../lib";

// ========================================================

// ========================================================

export default function ProductCard17({ product, bgWhite = false }) {
  const { slug, title, price, thumbnail, images, discount,  category } =
    product;

  return (
    <StyledCard elevation={0} bgWhite={bgWhite}>
      <ImageWrapper>
        <Discount discount={discount} />
        {/* <HoverActions product={product} /> */}

        <Link href={`/products/${slug}`} aria-label={`View ${title}`} className="link">
          <Image
            width={750}
            height={750}
            src={thumbnail}
            alt={`Thumbnail for ${title}`}
            className={images.length > 1 ? "thumbnail" : ""}
            sizes="(max-width: 468px) 385px, (max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            loading={images.length > 1 ? "lazy" : "eager"}
          />

          {images.length > 1 && (
            <Image
              width={750}
              height={750}
              src={images[1]}
              loading="lazy"
              className="hover-thumbnail"
              alt={`Hover thumbnail for ${title}`}
              sizes="(max-width: 468px) 385px, (max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            />
          )}
        </Link>
      </ImageWrapper>

      <ContentWrapper className="flex justify-center items-center flex-col">
        <Typography noWrap variant="body2" className="category">
          {category ? category : "N/A"}
        </Typography>

        <Link href={`/products/${slug}`} aria-label={`View ${title}`}>
          <Typography variant="h5" className="title">
            {title}
          </Typography>
        </Link>

        <Typography variant="subtitle1" color="primary" fontWeight={600}>
          {currency(price)}
        </Typography>
        <PurchaseBtn link={`/products/${slug}`} />
      </ContentWrapper>
    </StyledCard>
  );
}
